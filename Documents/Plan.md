...
1a.1. В открывшейся форме заполните поля следующими данными: <br>
 :green_circle: в поле `Номер карты` введите валидный номер карты со статусом "**Одобрена**" (**APPROVED**) <br>
 :green_circle: в поля `Месяц` и `Год` введите дату, соответствующую валидному диапазону (*см. таблицу входных данных*)  <br>
 :green_circle: в поле `Владелец` введите случайное валидное имя и фамилию на латинице <br>
 :green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:orange_circle: в поле `Номер карты` введите номер карты со статусом "**Отклонена**" (**DECLINED**) &ensp; &ensp; &ensp; :warning:*- Статус карты "**Отклонена**", поэтому банк должен отказать в покупке*<br>
:green_circle: в поле `Месяц` и `Год` введите дату, соответствующую валидному диапазону (*см. таблицу входных данных*)  <br>
:green_circle: в поле `Владелец` введите случайное валидное имя и фамилию на латинице <br>
:green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:green_circle: в поле `Год` введите число из валидного диапазона для этого поля (*см. таблицу входных данных*)  <br>
:green_circle: в поле `Владелец` введите случайное валидное имя и фамилию на латинице <br>
:green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:green_circle: в поле `Номер карты` введите валидный номер карты со статусом "**Одобрена**" (**APPROVED**) <br>
:green_circle: в поле `Месяц` введите валидный номер месяца <br>
...
:orange_circle: в поле `Номер карты` введите случайное число из 15 цифр  &ensp; &ensp; :warning: *- Длина номера карты меньше валидной, поэтому банк должен отказать в покупке*<br>
:green_circle: в поле `Месяц` и `Год` введите дату, соответствующую валидному диапазону (*см. таблицу входных данных*)  <br>
:green_circle: в поле `Владелец` введите случайное валидное имя и фамилию на латинице <br>
:green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:green_circle: в поле `Номер карты` введите валидный номер карты со статусом "**Одобрена**" (**APPROVED**) <br>
:orange_circle: в поле `Месяц` введите прошлый (относительно текущего) месяц  &ensp; &ensp; :warning: *- Сформированная (прошедшая) дата является невалидной,* <br>
:green_circle: в поле `Год` введите текущий год &ensp; &ensp; &ensp; &ensp; &ensp; &ensp;  &ensp; &ensp; &ensp; &ensp; &ensp; &ensp; &ensp; &ensp; &ensp; &ensp; &ensp; &ensp; &ensp; &ensp; :warning: *поэтому банк должен отказать в покупке*<br>
:green_circle: в поле `Владелец` введите случайное валидное имя и фамилию на латинице <br>
:green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:green_circle: в поле `Номер карты` введите валидный номер карты со статусом "**Одобрена**" (**APPROVED**) <br>
:orange_circle: в поле `Месяц` введите случайное, двухзначное число, больше 12 &ensp; &ensp; :warning: *- Количество месяцев в году не может быть больше 12*<br>
:green_circle: в поле `Год` введите число из валидного диапазона для этого поля (*см. таблицу входных данных*)  <br>
:green_circle: в поле `Владелец` введите случайное валидное имя и фамилию на латинице <br>
:green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:green_circle: в поле `Номер карты` введите валидный номер карты со статусом "**Одобрена**" (**APPROVED**) <br>
:green_circle: в поле `Месяц` введите валидный номер месяца <br>
:orange_circle: в поле `Год` введите число, большее чем "текущий год + 5" &ensp; &ensp; &ensp; :warning: *- Указанный год находится вне диапазона валидных дат, поэтому Банк должен отказать в покупке*<br>
:green_circle: в поле `Владелец` введите случайное валидное имя и фамилию на латинице <br>
:green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:green_circle: в поле `Номер карты` введите валидный номер карты со статусом "**Одобрена**" (**APPROVED**) <br>
:green_circle: в поле `Месяц` и `Год` введите дату, соответствующую валидному диапазону (*см. таблицу входных данных*)  <br>
:orange_circle: в поле `Владелец` введите имя на латинице, с добавлением спецсимволов и цифр &ensp; &ensp; &ensp; :warning: *- Спецсимволы и цифры не являются валидными для этого поля*<br>
:green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:green_circle: в поле `Номер карты` введите валидный номер карты со статусом "**Одобрена**" (**APPROVED**) <br>
:green_circle: в поле `Месяц` и `Год` введите дату, соответствующую валидному диапазону (*см. таблицу входных данных*)  <br>
:orange_circle: в поле `Владелец` введите один валидный символ (букву латиницы) &ensp; &ensp; &ensp; :warning: *- Количество символов меньше минимального валидного для этого поля*<br>
:green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:green_circle: в поле `Номер карты` введите валидный номер карты со статусом "**Одобрена**" (**APPROVED**) <br>
:green_circle: в поле `Месяц` и `Год` введите дату, соответствующую валидному диапазону (*см. таблицу входных данных*)  <br>
:orange_circle: в поле `Владелец` введите валидными символами имя и фамилию, общей длиной более 32 символов &ensp; &ensp; &ensp; :warning: *- Количество символов превышает валидный максимум для этого поля*<br>
:green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:green_circle: в поле `Месяц` и `Год` введите дату, соответствующую валидному диапазону (*см. таблицу входных данных*)  <br>
:green_circle: в поле `Владелец` введите случайное валидное имя и фамилию на латинице <br>
:orange_circle: в поле `CVC/CVV` введите **000** &ensp; &ensp; &ensp; :warning: *- Указанное число находится вне диапазона валидных данных  (*см. таблицу входных данных*)*<br>
...
:green_circle: в поле `Номер карты` введите валидный номер карты со статусом "**Одобрена**" (**APPROVED**) <br>
:green_circle: в поле `Месяц` введите валидный номер месяца <br>
:orange_circle: в поле `Год` введите прошлый (относительно текущего) год &ensp; &ensp; &ensp; :warning: *- Указанный год находится вне диапазона валидных дат, т.к уже прошел*<br>
:green_circle: в поле `Владелец` введите случайное валидное имя и фамилию на латинице <br>
:green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:green_circle: в поле `Номер карты` введите валидный номер карты со статусом "**Одобрена**" (**APPROVED**) <br>
:green_circle: в поле `Месяц` и `Год` введите дату, соответствующую валидному диапазону (*см. таблицу входных данных*)  <br>
:orange_circle: в поле `Владелец` введите имя и фамилию на кириллице с добавлением спецсимволов &ensp; &ensp; &ensp; :warning: *- Спецсимволы и буквы кириллицы не являются валидными для этого поля*<br>
:green_circle: в поле `CVC/CVV` введите случайное трехзначное число в валидном диапазоне (*см. таблицу входных данных*) <br>
...
:orange_circle:в поле `CVC/CVV` введите случайное двухзначное число &ensp; &ensp; :warning: *- Длина кода меньше валидной, поэтому банк должен отказать в покупке*<br>
